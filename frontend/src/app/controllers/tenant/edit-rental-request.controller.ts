...
import { EditRentalRequestModel } from '../../models/tenant/edit-rental-request.model';
...
@Component({
  ...
  templateUrl: '../../views/tenant/edit-rental-request.html',
  styleUrl: '../../views/tenant/edit-rental-request.css' 
})
export class EditRentalRequestController implements OnInit {
  ...
  constructor(
    ...
    private rentalRequestModel: EditRentalRequestModel // INJECTING THE MODEL HERE
  ) {}
  ...
  ngOnInit() {
    ...
    if (this.requestId) {
      // Use the Model to fetch existing request data
      this.rentalRequestModel.getRentalRequest(this.requestId).subscribe({
        next: (data: any) => {
  ...
  onSubmit(event: Event) {
    ...
    // Use the Model to update the request
    this.rentalRequestModel.updateRentalRequest(this.requestId, updateData).subscribe({
      next: () => {
